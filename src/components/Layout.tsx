...
export function Layout({ searchPlaceholder = 'Buscar...', children }: Props) {
  ...
  return (
    <div className="min-h-screen bg-[#f4f5fa] text-slate-800">
      <div className="flex min-h-screen">
        <aside className="flex w-60 flex-col border-r border-slate-200 bg-[#f7f8fc] p-5">
          <div>
            <p className="text-2xl font-bold text-violet-600">SprintPro</p>
            <p className="text-sm text-slate-500">{appUser?.organizationName ?? 'Workspace'}</p>
            ...
            {navItems.map((item) => (
              item.to === '#' ? (
                ...
                >
                  ...
                  <span>{item.label} <small>(em breve)</small></span>
                </p>
              ) : (
                <NavLink
                  key={item.to}
          ...
          <button
            type="button"
            onClick={() => logout()}
            className="mt-auto flex items-center justify-center gap-2 rounded-lg border border-slate-300 px-4 py-2 text-sm text-slate-600"
          >
        ...
        <main className="flex-1">
          <header className="border-b border-slate-200 bg-white px-6 py-4">
            <div className="flex items-center gap-3">
              <div className="flex flex-1 items-center gap-3 rounded-full border border-slate-200 bg-slate-50 px-4 py-2 text-sm">
                <span className="inline-block h-2.5 w-2.5 rounded-full bg-emerald-500" />
                <span className="font-medium text-slate-700">Pessoas online: {onlineUsers.length}</span>
                  ...
                  {onlineUsers.slice(0, 4).map((user) => {
                    ...
                    return (
                      <span
                        ...
                        className="grid h-7 w-7 place-items-center rounded-full bg-violet-100 text-[10px] font-semibold text-violet-700"
                      >
                        {userInitials}
                      </span>
                    )
              ...
              <button
                type="button"
                title="Tema (em breve)"
                className="grid h-11 w-11 place-items-center rounded-md bg-[#f4f5f8] text-slate-600"
              >
                <svg viewBox="0 0 24 24" className="h-6 w-6" fill="none" stroke="currentColor" strokeWidth="2">
                  <path d="M21 12.8A9 9 0 1 1 11.2 3a7 7 0 0 0 9.8 9.8Z" />
                </svg>
              </button>
              <div className="flex min-w-[220px] items-center justify-end gap-3 border-l border-slate-200 pl-5">
                <div className="text-right">
                  <p className="text-sm font-semibold text-slate-800">{appUser?.fullName ?? 'Usuário'}</p>
                  <p className="text-xs text-slate-500">{userRoleLabel(appUser?.role)}</p>
                </div>
